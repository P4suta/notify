...
| Actions and attachments | Implemented | Action parsing and 10-character action IDs are in the differential corpus. Attachment publish, download, range, quota, expiry, and adapters exist, but the attachment port still buffers complete objects and has not yet moved to the required `begin/write/finish/abort` streaming contract. |
