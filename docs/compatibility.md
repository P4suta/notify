...
| HTTP/3 transport | Contract | Mist remains the compatibility path for HTTP/1.1, HTTP/2, and RFC 6455 Upgrade. With local TLS, HTTP/3 uses the same URL and numeric UDP port and is discovered through live-state Alt-Svc. Publish, poll, live JSON, and RFC 9220 WebSocket loopback contracts share the same router and broker payloads. The only normalized handshake difference is HTTP/1.1 WebSocket 101 versus HTTP/3 Extended CONNECT 200. The full 153-case differential corpus and browser matrix have not yet been duplicated over HTTP/3, so transport-wide equivalence beyond those contracts is not claimed. |
